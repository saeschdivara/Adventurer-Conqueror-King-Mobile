 ...
 */
public class AncientDragon extends Npc {
        ...
        while (powerList.size() < amount) {
            ...
            switch (random) {
                ...
                case 8:
                    ...
                    if (routine.remove("Claw 1D12,Claw 1D12,Bite 4D10")) {
                        routine.add("Claw 3D4,Claw 3D4,Bite 5D8");
                    }else if (routine.remove("Claw 3D4,Claw 3D4,Bite 5D8")) {
    ...
    private static List<Npc> createMonster(int groupSize) {
        ...
        for (int i = 1; i <= groupSize; i++) {
            AncientDragon monster = new AncientDragon();
            monster.addToAttackRoutine("Claw 1D12,Claw 1D12,Bite 4D10");
...
//            monster.addExtraInformation(BaseDragon.awake(5)); //Values in the Core book depict awake not asleep!
